...
(function (global, factory) {
                ...
                var thenPromise = newPromise(function (resolve, reject) {
                    _queue.push(function () {
                        try {
                            ...
                            var value = tsUtils.isFunction(handler) ? handler(_settledValue) : _settledValue;
                            if (tsUtils.isPromiseLike(value)) {
                                value.then(resolve, reject);
                            }
                            else if (tsUtils.isFunction(handler)) {
                                resolve(value);
                            }
                            else if (_state === 3 ) {
                                reject(value);
    ...
    function _normalizeTimeoutValue(timeout, defaultTimeout) {
        ...
        if (!tsUtils.isUndefined(timeout)) {
            if (tsUtils.isNumber(timeout)) {
                result = timeout;
            }
            else {
                while (!tsUtils.isUndefined(timeout) && tsUtils.isArray(timeout) && timeout.length > 0) {
                    timeout = timeout[0];
                    if (tsUtils.isNumber(timeout)) {
                        result = timeout;
                        break;
    ...
    function timeoutItemProcessor(timeout) {
        ...
        var hasTimeout = tsUtils.isNumber(timeoutValue);
        ...
        return function (pending) {
            ...
            if (hasTimeout && callbackTimeout > 0) {
                tsUtils.scheduleTimeout(_processPending, callbackTimeout);
            }
            else if (_isFakeTimersEnabled()) {
                tsUtils.scheduleTimeout(_processPending, 0);
            }
            else {
                tsUtils.scheduleMicrotask(_processPending);
            }
        };
    }
    ...
    function createIdlePromise(executor, timeout) {
        var options;
        var theTimeout = _normalizeTimeoutValue(timeout, _defaultIdleTimeout);
        if (tsUtils.isNumber(theTimeout) && theTimeout >= 0) {
            options = {
                timeout: theTimeout
            };
        }
        var processor = function (pending) {
