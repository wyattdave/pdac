...
Each report section shows downloads from its most recent completed run only. A **Run both** result can contain its matching raw and totals workbooks, but older runs are not displayed.

The **Weekly report** tracks created and modified solutions, together with their agents, canvas apps, code apps, model-driven apps, flows, and Dataverse tables. It retains three months locally, supports the same solution publisher/ownership filters plus an environment filter, and downloads a self-contained HTML report suitable for printing or saving as PDF. Each period uses one sortable solution table with an All/Deployed/Updated filter and keeps both primary-component charts alongside it. The table scroll area matches the combined chart height, long solution names wrap, and the primary type appears beside the version to preserve space for component columns. The downloaded report has an A4 portrait print layout that retains the side-by-side charts, preserves chart colours, and avoids splitting chart cards or individual table rows across pages. Solution rows show small **Deployed** and **Updated** indicators; an update is counted only when modification occurs on a different calendar day from creation, avoiding import-processing timestamp differences being counted twice. Opening the report displays its dedicated local event and component stores first and selects the newest populated week. A Solutions report saved to trend data also populates those weekly stores from the solution rows and components it already collected, avoiding another initial solution inventory call. When that local source is unavailable, turning on **Track weekly solution changes** performs the initial three-month backfill once. Each manual refresh and hourly check then resumes from the per-environment poll cursor saved locally, with a short overlap to cover boundary changes or days when PDAC was not running; it does not repeat a daily three-month solution query. The status identifies how many saved events were loaded and whether the Solutions report filters hide any of them. The Chrome extension stores events and components in two IndexedDB stores; the Node server uses two tables in `report-trends.sqlite`. Dataverse throttling responses are retried using `Retry-After`.

The Settings tab can export, clear, and import local trend data. The combined export is generated from the import template itself, so its worksheet set, names, headers, and instructions match exactly and it can be imported into another PDAC instance without editing. Use **Download import template** for a blank workbook, keep its worksheet names and headers unchanged, enter rows, then choose **Import Excel**. For each report, account, and imported date, the imported rows replace the complete existing date snapshot so duplicate dates cannot be counted twice. Imported data retains the same two-year history policy as generated trend data.
...
- **Reports**: run AI Flow, Agent Sessions, Solutions, and Flow Runs reports across selected environments; generate totals and raw workbooks; queue manual runs; schedule daily runs; auto-download completed files; restore cached download links; and review or download the three-month weekly solution-change report.
