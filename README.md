...
The **Weekly report** tracks created and modified solutions, together with their agents, canvas apps, code apps, model-driven apps, flows, and Dataverse tables. It retains three months locally, supports the same solution publisher/ownership filters plus an environment filter, and downloads a self-contained HTML report suitable for printing or saving as PDF. Opening the report displays local data first. Enabled tracking checks Dataverse hourly and normally queries only changes since the last successful environment sync, with a short overlap to avoid missing boundary updates. The initial check and one recovery check per day reconcile solution headers across the full retention window, but component details are requested only for missing or changed solution events. The Chrome extension stores events and components in two IndexedDB stores; the Node server uses two tables in `report-trends.sqlite`. Dataverse throttling responses are retried using `Retry-After`.
